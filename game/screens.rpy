...
# MCAS specific styles

style mcas_init_text:
    yalign 0.5
    font "mod_assets/ProsperoNbpRegular-3jJ3.ttf"
    size 30
    color "#dddd00"
    outlines []

    hover_xoffset -3
    hover_outlines [(3, "#ffd", 0, 0), (2, "#ff8", 0, 0), (1, "#ff0", 0, 0)]
...
screen say(who, what):
    ...
    use sc_quick_menu # MCAS
    use main_hub_quick_menu
    use quick_menu
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        if persistent.epiphany_level==0:
            hbox:
                style_prefix "quick"
...
init python:
    def FinishEnterNameStalin(): # DO NOT SPOIL THIS TO SAVAGESENTRAL (the Stalin DokiTuber)
        if not player: return
        renpy.hide_screen("name_input")
        if "stalin" in player.lower():
            if "_" in player:
                renpy.show_screen("dialog", message="Welcome, SavageSentral.",ok_action=Function(FinishEnterName))
            else:
                renpy.show_screen("name_input", message="You can use underscores in this one,\nSavageSentral.", ok_action=Function(FinishEnterName))
            return
        ...
        vbox:
            ...
            if not persistent.autoload or not main_menu:
                ...
                if main_menu:
                    ...
                    textbutton _("New Game") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Enter your name:", ok_action=Function(FinishEnterNameStalin)))
...
screen name_input(message, ok_action):
    ...
    frame:
        ...
        vbox:
            ...
            input default "" value VariableInputValue("player") length 20 allow "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz _"
            # You're welcome, SavageSentral/
            #hbox:
            #    xalign 0.5
